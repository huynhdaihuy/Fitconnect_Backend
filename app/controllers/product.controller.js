...
const updateProduct = asyncHandler(async(req, res) => {
    try {
        ...
        const urlImg = [];
        const files = req.files;
        const objUpdated = req.body;
        const removeStr = JSON.stringify(objUpdated).replace(/\[/g, '.').replace(/\]/g, '');
        const objectFinal = JSON.parse(removeStr)
        if (files) {
            if (Array.isArray(files.images)) {
                for (const file of files.images) {
                    const path = file.tempFilePath;
        ...
        if (urlImg.length > 0) {
            objectFinal.images = urlImg[0].url
        }
        const updateProduct = await Product.findByIdAndUpdate(id, objectFinal, {
            new: true,
        });
